...
import { getApproveInstructor ,approveInstructor,cancelApprovel} from '@/app/lib/api/adminApi';
...
export default function InstructorApproval() {
     const [instructorData , setInstructorData] = useState<InstructorData[]>([]);
        ...
        const handleApprove = async(id:string | undefined) => {
            ...
            if(res.data.response.success === true) {
                setInstructorData((prev) =>
                    prev.map((inst) =>
                      inst.id === id ? { ...inst, isApproved: true } : inst
                    )
                  );                
        ...
        const handleCancelApproval = async(id:string|undefined) => {
            const res = await cancelApprovel(id);
            if (res.data.response.success) {
                toast.success(res.data.response.message);
          
                setInstructorData((prev) =>
                  prev.map((inst) =>
                    inst.id === id ? { ...inst, isApproved: false } : inst
                  )
                );
              }
        }
        ...
        <>
          <ToastContainer />
              <div className="h-screen ">
                ...
                <div className='mt-10'>
                <h3 className='text-2xl ml-7 mb-4 font-semibold'>Instructor Approval</h3>
                        ...
                        <TableRow key={instructor.id}>
                           <TableCell className="flex justify-center items-center">
                            ...
                            {!instructor.isApproved ? (
                                ...
                                onClick={() => handleCancelApproval(instructor.id)}
                                >
                                Cancel Approval
                                </button>
                            )}
